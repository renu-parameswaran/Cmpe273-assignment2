...
def handler(event, context):
    ...
    if all(key in event for key in keys):
        ...
        try:
            ...
            menu_table = boto3.resource('dynamodb', region_name='us-west-2').Table('pizzashopenv')
        ...
        if order.get('selection') == 'empty':
            ...
            selection = menu_table.get_item(Key={'pizzaid': menu_id}).get('Item').get('selection')[
                int(event.get('input')) - 1]
            order['selection'] = selection
            sizes = menu_table.get_item(Key={'pizzaid': menu_id}).get('Item').get('size')
            ...
            size = menu_table.get_item(Key={'pizzaid': menu_id}).get('Item').get('size')[int(event.get('input')) - 1]
            order['size'] = size
            cost = menu_table.get_item(Key={'pizzaid': menu_id}).get('Item').get('price')[int(event.get('input')) - 1]
            order['cost'] = cost
            response['Message'] = 'Your order costs $' + str(
                cost) + '. We will email you when the order is ready. Thank you!'
            order['order_time'] = time.strftime("%m-%d-%Y@%H:%M:%S")
        else:
            response[
                'Message'] = 'Your order has already been placed. We will email you when the order is ready. Thank you!'
            return response
